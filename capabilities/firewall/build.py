#!/usr/bin/env python3
"""
Build the Prompt Injection Firewall model from JSONL exemplar data.

Usage:
    python build.py
    python build.py --output path/to/output.ada
"""
...
DEFAULT_OUTPUT = MODEL_DIR / "firewall.ada"
...
def build(output_path: Path | None = None) -> None:
    ...
    for r in records:
        ...
        family = r["metadata"]["attack_family"]
        label_counts[label] = label_counts.get(label, 0) + 1
        family_counts[family] = family_counts.get(family, 0) + 1

    print(f"\nLabel distribution ({len(label_counts)} labels):")
    for label, count in sorted(label_counts.items()):
        print(f"  {label}: {count} exemplars")

    print(f"\nAttack family distribution ({len(family_counts)} families):")
    for family, count in sorted(family_counts.items()):
        print(f"  {family}: {count} exemplars")

    print(f"\nTotal records: {len(records)}")
    print(f"\nReady to package as {output}")
    print("(Packaging requires the Ada runtime SDK)")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build Prompt Injection Firewall model")
