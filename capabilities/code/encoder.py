"""
Encoder for the Ada Code model — file-level codebase intelligence.
...
def file_to_record(
    ...
    """Convert a source file to a Ada build record.
...
async def handle_mcp_tool(tool_name: str, arguments: dict, context: dict) -> dict:
    ...
    """
    handlers = {
        "ada_search": _handle_search,
        "ada_related": _handle_related,
        "ada_context": _handle_context,
        "ada_stats": _handle_stats,
        "ada_drift": _handle_drift,
        "ada_risk": _handle_risk,
        "ada_session_write": _handle_session_write,
        "ada_session_recall": _handle_session_recall,
    }
...
async def _handle_search(arguments: dict, context: dict) -> dict:
    ...
    if not coarse_results:
        return {
            ...
            "confidence": 0.0,
            "match_method": "none",
            "error": "No files indexed. Run: ada-code compile .",
        }
...
async def _handle_related(arguments: dict, context: dict) -> dict:
    ...
    if glyph_row is None:
        return {
            ...
            "confidence": 0.0,
            "match_method": "none",
            "error": f"File not in index: {file_path}. Run: ada-code compile .",
        }
...
async def _handle_drift(arguments: dict, context: dict) -> dict:
    ...
    # 3. Compute drift
    from drift import compute_drift, drift_label
...
async def _handle_risk(arguments: dict, context: dict) -> dict:
    ...
    from drift import score_commit_files
