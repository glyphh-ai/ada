"""
Build pipeline for ada-code.

Walks a repository, encodes each source file into an HDC vector,
and posts records to the Ada runtime listener for storage in pgvector.
...
GLYPHH_CONFIG_FILE = Path.home() / ".ada" / "config.json"


def _load_cli_config() -> dict:
    """Load the Ada CLI config (~/.ada/config.json)."""
...
def _resolve_token(explicit: str | None, runtime_url: str) -> str | None:
    ...
    print("Warning: no --token provided and no CLI session found. Run: ada auth login")
    return None
...
def _resolve_org_id(explicit: str | None, runtime_url: str) -> str:
    ...
    print("Error: no org_id available. Run: ada auth login")
    sys.exit(1)
...
def compile_repo(
    ...
    """Compile a repository into the Ada code index.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compile a repository into the Ada code index"
    )
